...
import com.bth.models.player.Friend;
import com.bth.models.player.Fusion;
...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            String duahau = dataArray.toJSONString();
            ...
            dataArray.add(0);  //diemdanhbossst
            dataArray.add(0); //diemdanhbosstt
            String gioto = dataArray.toJSONString();
            dataArray.clear();
            ...
            dataArray.add(0); //20sd
            dataArray.add(0); //20hp
            dataArray.add(0); //20ki
            String qua8_3 = dataArray.toJSONString();
            dataArray.clear();
            ...
            GirlkunDB.executeUpdate("insert into player"
                            ...
                            + "data_black_ball, data_side_task, timequa8_3, data_card,data_dua_hau,data_diem_gioto,archivement) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, qua8_3, data_card,duahau,gioto,archivement);
    ...
    public static void updatePlayer(Player player) {
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                //data item time
                dataArray.add((player.itemTime.sd20pt8_3 ? (ItemTime.MUOILAMPHUT - (System.currentTimeMillis() - player.itemTime.lastsd20pt8_3)) : 0));
                dataArray.add((player.itemTime.hp20pt8_3 ? (ItemTime.MUOILAMPHUT - (System.currentTimeMillis() - player.itemTime.lasthp20pt8_3)) : 0));
                dataArray.add((player.itemTime.ki20pt8_3 ? (ItemTime.MUOILAMPHUT - (System.currentTimeMillis() - player.itemTime.lastki20pt8_3)) : 0));
                String qua8_3 = dataArray.toJSONString();
                dataArray.clear();
                ...
                //data boss điểm boss giỗ tổ
                dataArray.add(player.diemsontinh);
                dataArray.add(player.diemthuytinh);
                String tieudietbossgioto = dataArray.toJSONString();
                dataArray.clear();
                ...
                String query = " update player set head = ?, have_tennis_space_ship = ?,"
                        ...
                        + "enemies = ?, data_intrinsic = ?, data_item_time = ?, timequa8_3 = ?, data_task = ?, data_mabu_egg = ?, pet = ?,"
                        + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?, skills_shortcut = ?, pointPvp=? ,"
                        + "diemtanghoa8_3 = ?, NguHanhSonPoint = ?, power = ?, thoivang = ?, goldbag = ?, data_card = ?, gdvip = ?, data_dua_hau = ?, data_diem_gioto = ? where id = ?";
...
//                Logger.success("Total time save player " + player.name + " thành công! " + (System.currentTimeMillis() - st) + "\n");
    ...
    public static boolean diemnaptien(Player player, int num) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update player set diemnaptien = (diemnaptien + ?) where id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().userId);
            ps.executeUpdate();
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi update điểm nạp tiền " + player.name);
        } finally {
            try {
                ps.close();
            } catch (SQLException ex) {
