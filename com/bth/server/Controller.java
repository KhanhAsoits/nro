...
import com.bth.services.func.UseItem;
...
import com.bth.services.func.LuckyRound;
...
//        Service.getInstance().sendThongBaoOK(session, "Vui lòng đăng ký tài khoản tại trang chủ NROKURO.COM!");
    }
    ...
    public void sendInfo(MySession session) {
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            NpcService.gI().createTutorial(player, -1,
                    "Chào mừng " + player.name + " đến với Ngọc Rồng Đấu La\n"
    ...
    private void sendThongBaoServer(Player player) {
        Service.getInstance().sendThongBaoFromAdmin(player, "DragonBoy Đấu La.\nMáy chủ thử nghiệm !!");
    }

    private void clearVTSK(Player player) {
        player.inventory.itemsBag.stream().filter(item -> item.isNotNullItem() && item.template.id == 1112).forEach(item -> {
            InventoryServiceNew.gI().subQuantityItemsBag(player, item, item.quantity);
        });
        player.inventory.itemsBox.stream().filter(item -> item.isNotNullItem() && item.template.id == 1112).forEach(item -> {
            InventoryServiceNew.gI().subQuantityItemsBox(player, item, item.quantity);
        });
        player.inventory.itemsBody.stream().filter(item -> item.isNotNullItem() && item.template.id == 1112).forEach(item -> {
