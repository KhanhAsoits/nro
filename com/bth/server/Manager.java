...
import com.bth.jdbc.daos.ShopDAO;
import com.bth.kygui.ItemKyGui;
import com.bth.kygui.ShopKyGuiManager;
...
import com.bth.models.intrinsic.Intrinsic;
import com.bth.models.item.Item;
import com.bth.models.map.WayPoint;
import com.bth.models.map.daihoilanthu23.TrongTai;
...
import com.bth.models.reward.ItemOptionMobReward;
import com.bth.models.reward.MobReward;
import com.bth.models.shop.Shop;
import com.bth.models.skill.NClass;
import com.bth.models.skill.Skill;
import com.bth.models.task.SideTaskTemplate;
import com.bth.models.task.SubTaskMain;
...
public class Manager {
    ...
    public static final List<com.bth.models.map.Map> MAPS = new ArrayList<>();
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            com.bth.models.map.Map map = new com.bth.models.map.Map(mapTemp.id,
        ...
        TrongTai e = new TrongTai();
        e.initTrongTai();
        Logger.success("Init trọng tài đhvt 23 thành công!\n");
    }
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            while (rs.next()) {
             ...
             Logger.log(Logger.PURPLE, "Tải dữ liệu ký gửi: [" +ShopKyGuiManager.gI().listItem.size()+"]!\n" );
            ...
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (item.getTemp().id == 12) {
                            //   item.setRatio(new int[]{Integer.parseInt(ratio[0]), Integer.parseInt(ratio[1]) / 4 * 3});
                        }
                        if (item.getTemp().id == 17) {
                            //   item.setRatio(new int[]{Integer.parseInt(ratio[0]), Integer.parseInt(ratio[1]) / 4 * 3});
                        }
...
//                    dataArray.clear();
        ...
        Logger.log(Logger.GREEN, "Tổng thời gian load database: " + (System.currentTimeMillis() - st) + "(ms)\n");
    }
    public void loadProperties() throws IOException {
        Properties properties = new Properties();
        properties.load(new FileInputStream("data/girlkun/properties"));
        Object value = null;
        //###Config sv
        if ((value = properties.get("server.bth.port")) != null) {
            ServerManager.PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.bth.name")) != null) {
            ServerManager.NAME = String.valueOf(value);
        }
        if ((value = properties.get("server.bth.sv")) != null) {
            SERVER = Byte.parseByte(String.valueOf(value));
        }
        String linkServer = "";
        for (int i = 1; i <= 10; i++) {
            value = properties.get("server.bth.sv" + i);
            if (value != null) {
                linkServer += String.valueOf(value) + ":0,";
            }
        }
        DataGame.LINK_IP_PORT = linkServer.substring(0, linkServer.length() - 1);
        if ((value = properties.get("server.bth.waitlogin")) != null) {
            SECOND_WAIT_LOGIN = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.bth.maxperip")) != null) {
            MAX_PER_IP = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.bth.maxplayer")) != null) {
            MAX_PLAYER = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server.bth.expserver")) != null) {
            RATE_EXP_SERVER = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server.bth.local")) != null) {
            LOCAL = String.valueOf(value).toLowerCase().equals("true");
        }
        if ((value = properties.get("server.bth.skien")) != null) {
            SUKIEN = Byte.parseByte(String.valueOf(value));
        }
    }
