...
public class PlayerService {
    ...
    private static final int COST_GEM_HOI_SINH = 1000;
        ...
        if (player.isDie()) {
            ...
            if( MapService.gI().isMapTatCo(player.zone.map.mapId)){
                if (player.inventory.gold >= 5000000000L) {
                    ...
                    Service.getInstance().sendThongBao(player, "Không thể hồi sinh");
                    return;
                }
            }
            if (MapService.gI().isMapBlackBallWar(player.zone.map.mapId)) {
                ...
                if (player.inventory.gem > COST_GEM_HOI_SINH) {
                    ...
                    Service.getInstance().sendThongBao(player, "Không đủ ngọc để thực hiện, còn thiếu " + Util.numberToMoney(COST_GEM_HOI_SINH
