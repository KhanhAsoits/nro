...
public class RewardService {
    ...
    public List<Item> getListItemLuckyRound(Player player, int num) {
        ...
        for (int i = 0; i < num; i++) {
            Template.ItemLuckyRound item = Manager.LUCKY_ROUND_REWARDS.get(Util.nextInt(0, Manager.LUCKY_ROUND_REWARDS.size() - 1));
            if (item.temp.gender == player.gender || item.temp.gender > 2) {
                if (Util.isTrue(item.ratio, item.typeRatio)) {
                    ...
                    for (Template.ItemOptionLuckyRound io : item.itemOptions) {
                        int param = 0;
                        if (io.param2 != -1) {
