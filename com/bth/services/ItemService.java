...
public class ItemService {
    ...
    public void openxocavang(Player player, Item itemUse) {
        try {
            ...
            if (rd <= 80) {
                ...
                item = minipetvip(true);
            } else if (rd <= 10) {
                item = ctvip(true);
            }
            if (item.template.id == 861) {
                item.quantity = Util.nextInt(100, 500);
            }
    ...
    public void openxocaxanh(Player player, Item itemUse) {
        try {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 1) {
                Service.getInstance().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
                return;
    ...
    public Template.ItemOptionTemplate getItemOptionTemplate(int id) {
    ...
    public void Openquatanthu(Player player, Item itemUse) {
        try {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 1) {
                Service.getInstance().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
                return;
    ...
    public void OpenSkHungVuong(Player player, Item itemUse) {
        try {
            if (InventoryServiceNew.gI().getCountEmptyBag(player) <= 1) {
                Service.getInstance().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
                return;
            ...
            icon[1] = item.template.iconID;
            InventoryServiceNew.gI().subQuantityItemsBag(player, itemUse, 1);
            Service.getInstance().addSMTN(player, (byte) 2, 1000000, false);
            InventoryServiceNew.gI().addItemBag(player, item);
            InventoryServiceNew.gI().sendItemBags(player);
            player.inventory.event++;
            Service.getInstance().sendThongBao(player, "Bạn đã nhận được " + item.template.name);
    ...
    public int optionIdSKH(int skhId) {
        switch (skhId) {
            case 127: //Set BTH Taiyoken
                return 139;
            case 128: //Set BTH Genki
                return 140;
            case 129: //Set BTH Kamejoko
                return 141;
            case 130: //Set BTH KI
                return 142;
            case 131: //Set BTH Dame
                return 143;
            case 132: //Set BTH Summon
                return 144;
            case 133: //Set BTH Galick
                return 136;
            case 134: //Set BTH Monkey
                return 137;
            case 135: //Set BTH HP
                return 138;
        }
    ...
    public Item minipetcui(boolean rating) {
        short[] minipet = new short[]{1046,942,943,944,908,909,910};
        Item item = createItemSetKichHoat(minipet[Util.nextInt(minipet.length - 1)], 1);
        item.itemOptions.add(new Item.ItemOption(50, Util.nextInt(11,20)));//sd
        item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(20,40)));//hp
        item.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5,10)));//giap
        item.itemOptions.add(new Item.ItemOption(14, Util.nextInt(1,5)));//chi mang
        item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(3) + 1));//hsd
        return item;
    }
    public Item minipetvip(boolean rating) {
        short[] minipet = new short[]{1046,942,943,944,908,909,910};
        Item item = createItemSetKichHoat(minipet[Util.nextInt(minipet.length - 1)], 1);
        item.itemOptions.add(new Item.ItemOption(50, Util.nextInt(11,20)));//sd
        item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(20,20)));//hp
        item.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5,10)));//giap
        item.itemOptions.add(new Item.ItemOption(14, Util.nextInt(1,5)));//chi mang
        item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(20) + 10));//hsd
        return item;
    }

    public Item ctcui(boolean rating) {
        short[] ct = new short[]{1010,1011,1012,1087,1088,1089,1090,1091};
        Item item = createItemSetKichHoat(ct[Util.nextInt(ct.length - 1)], 1);
        item.itemOptions.add(new Item.ItemOption(50, Util.nextInt(11,20)));//sd
        item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(20,40)));//hp
        item.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5,10)));//giap
        item.itemOptions.add(new Item.ItemOption(14, Util.nextInt(1,5)));//chi mang
    ...
    public Item ctvip(boolean rating) {
        int[] ct = new int[]{1010,1011,1012,1087,1088,1089,1090,1091};
        Item item = createItemSetKichHoat(ct[Util.nextInt(ct.length - 1)], 1);
        item.itemOptions.add(new Item.ItemOption(50, Util.nextInt(11,20)));//sd
        item.itemOptions.add(new Item.ItemOption(77, Util.nextInt(20,20)));//hp
        item.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5,10)));//giap
    ...
    public Item thoivang() {
        Item item = createItemSetKichHoat(457, 1);
        item.itemOptions.add(new Item.ItemOption(30, 1));//ko the gd
        return item;
    }
    ...
    public Item DoThienSu(int itemId, int gender) {
        ...
        if (Util.isTrue(80, 100)) {
            if (quan.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(22, Util.highlightsItem(gender == 0, new Random().nextInt(11) + 200))); // hp 120k-130k
            }
        } else {
            if (quan.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(22, Util.highlightsItem(gender == 0, new Random().nextInt(21) + 220))); // hp 130-150k 15%
            }
        }
        //găng
        if (Util.isTrue(80, 100)) {
            if (gang.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(0, Util.highlightsItem(gender == 2, new Random().nextInt(651) + 15000))); // 9350-10000
            }
        } else {
            if (gang.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(0, Util.highlightsItem(gender == 2, new Random().nextInt(1001) + 16000))); // gang 15% 10-11k -xayda 12k1
            }
        }
        //giày
        if (Util.isTrue(80, 100)) {
            if (giay.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(23, Util.highlightsItem(gender == 1, new Random().nextInt(21) + 150))); // ki 90-110k
            }
        } else {
            if (giay.contains(itemId)) {
                dots.itemOptions.add(new Item.ItemOption(23, Util.highlightsItem(gender == 1, new Random().nextInt(21) + 180))); // ki 110-130k
            }
        }

        if (nhan.contains(itemId)) {
            dots.itemOptions.add(new Item.ItemOption(14, Util.highlightsItem(gender == 1, new Random().nextInt(3) + 30))); // nhẫn 18-20%
        }
