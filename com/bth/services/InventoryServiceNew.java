...
public class InventoryServiceNew {
    ...
    public Item findnr2s (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 15)) {
                return item;
            }
        }
        return null;
    ...
    public Item findbdkb (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 611)) {
                return item;
            }
        }
        return null;
    }
    public Item findnlsktet (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 748)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    }
    public Item findnlsktet1 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 749)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    }
    public Item findnlsktet2 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 750)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    }
    public Item findnlsktet3 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 751)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    ...
    public Item findnlsktet4 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1002)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    ...
    public Item findnlsktet5 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1003)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    }
    public Item findnlsktet6 (Player pl) {
        for (Item item : pl.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1004)&&(item.quantity >=99) ) {
                return item;
            }
        }
        return null;
    ...
    private boolean addItemSpecial(Player player, Item item) {
        ...
        switch (item.template.id) {
            case 568: //quả trứng
                ...
                return true;
            case 569: //quả dưa hấu
                if (player.duahau == null) {
                    DuaHau.createduahau(player);
                    Service.getInstance().sendThongBao(player,"Bạn vừa trồng cây dưa hấu ở nhà");
                    return true;
                }else{
                    return false;
                }
    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                if (it.template.id == 1035) {
                    int add = 10000 - it.quantity;
                    if (itemAdd.quantity <= add) {
                        it.quantity += itemAdd.quantity;
                        itemAdd.quantity = 0;
                        return true;
    ...
    public boolean finditemRuongGo(Player player) {
