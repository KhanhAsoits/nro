...
//                    break;
    ...
    public void checkDoneTaskPickItem(Player player, ItemMap item) {
        if (!player.isBoss && !player.isPet && !player.isNewPet && item != null) {
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_20_6:
                    npcSay(player, ConstNpc.CUI,
                            "Chơi game quá 180p mỗi ngày sẽ có hại cho sức khỏe\n"
                            + "Chơi game quá 180p mỗi ngày sẽ có hại cho sức khỏe\n"
                            + "Chơi game quá 180p mỗi ngày sẽ có hại cho sức khỏe");
                    break;
                //--------------------------------------------------------------
...
//        }
    ...
    public int getIdTask(Player player) {
        if (player.isPet || player.isNewPet || player.isBoss || player.playerTask == null || player.playerTask.taskMain == null) {
            return -1;
        }
