...
import com.bth.jdbc.daos.GodGK;
...
public class SummonDragon {
    ...
    private final int timeResummonShenron = 100000;
    ...
    public void summonNamec(Player pl) {
        if (pl.zone.map.mapId == 7 ) {
            ...
            activeShenron(pl, true,SummonDragon.DRAGON_PORUNGA);
                EffectMapService.gI().sendEffectMapToAllInMap(pl.zone,
                        49, 2, 100, pl.location.x, pl.location.y, 1);
            sendWhishesNamec(pl);
        } else {
    ...
    private void sendWhishesNamec(Player pl) {
        NpcService.gI().createMenuRongThieng(pl, ConstNpc.NAMEC_1, "Ta sẽ ban cho cả bang ngươi 1 điều ước, ngươi có 5 phút, hãy suy nghĩ thật kỹ trước khi quyết định", "x99 ngọc rồng 5 sao");
    }
            ...
            case ConstNpc.SHENRON_1_2:
                switch (this.select) {
                    ...
                    case 1: //+1,5 ngọc
                        ...
                        break;
                    case 2: //+200 tr smtn
                        if (this.playerSummonShenron.nPoint.power <= 90000000000L) {
                            Service.getInstance().addSMTN(this.playerSummonShenron, (byte) 2, 200000000, false);
                        } else {
                            Service.getInstance().sendThongBao(playerSummonShenron, "Xin lỗi sức mạnh của ngươi đã vượt ngoài sự am hiểu của ta,khiến điều ước này khó quá, ta không thể thực hiện.");
                            reOpenShenronWishes(playerSummonShenron);
                            return;
                        }
                        break;
