...
//                }
        ...
        if (this.isMapKhiGa(mapId)) {
            if (player.clan == null || player.clan.khiga == null) {
                return null;
            }
            if (this.isMapKhiGa(player.zone.map.mapId)) {
                for (Mob mob : player.zone.mobs) {
                    if (!mob.isDie()) {
                        return null;
                    }
...
//                }
            ...
            return player.clan.khiga.getMapById(mapId);
        }
        //**********************************************************************
    ...
    public boolean isMapbahatmit(int mapId) {
        for (Map map : Manager.MAPS) {
            if (map.mapId == mapId) {
                return map.type == ConstMap.MAP_BAHATMIT;
            }
        }
        return false;
    }

    public boolean isMapTatCo(int mapId) {
        for (Map map : Manager.MAPS) {
            if (map.mapId == mapId) {
                return map.type == ConstMap.MAP_TAT_CO;
            }
        }
        return false;
    }
    ...
    public boolean isMapKhiGa(int mapId) {
        return mapId >= 149 && mapId <= 152;
    }
