...
import com.bth.services.func.Input;
import com.bth.utils.FileIO;
...
public class Service {
    ...
    public void UpdatePlayerAuto() {
        try {
            Player player = null;
            int i;
            synchronized (Client.gI().getPlayers()) {
                for (i = 0; i < Client.gI().getPlayers().size(); ++i) {
            ...
            //Logger.log(Logger.YELLOW,"AUTO SAVE PLAYER DATA SUCCESS :"+Client.gI().getPlayers().size()+"\n");
            ClanService.gI().saveclanauto();
            Thread.sleep(1000);
...
//            e.printStackTrace();
    ...
    public void regisAccount(Session session, Message _msg) {
        try {
            ...
            if (!(user.length() <= 18)) {
                sendThongBaoOK((MySession) session, "Tài khoản phải có độ dài nhỏ hơn 18 ký tự");
                return;
            }
            if (!(pass.length() <= 18)) {
                sendThongBaoOK((MySession) session, "Mật khẩu phải có độ dài nhỏ hơn 18 ký tự");
                return;
            }
    ...
    public void chat(Player player, String text) {
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("admin")) {
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1, "Quyền năng của ADMIN Đấu La: " + "Cư dân online: " + Client.gI().getPlayers().size() + " Thread: " + Thread.activeCount() + "\n",
                        "Server\nMaintenance",
                        "User Server",
                        "Event Server",
                        "Exp Server",
...
//                    ItemShop it = new Shop().getItemShop(itemId);
                ...
                Input.gI().createFormchatserver(player);
...
//    }
    ...
    public void charDie(Player pl) {
        ...
        try {
            ...
            if (!pl.isPet && !pl.isNewPet && !pl.isBoss && pl.idNRNM != -1) {
...
//            Send_Info_NV(pl);
    ...
    public void chooseFlag(Player pl, int index) {
        ...
        if (MapService.gI().isMapTatCo(pl.zone.map.mapId)) {
            sendThongBao(pl, "Không thể đổi cờ trong map này!");
            return;
        }
