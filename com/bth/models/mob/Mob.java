...
import com.bth.models.player.Location;
import com.bth.models.player.Pet;
import com.bth.models.player.Player;
import com.bth.models.reward.ItemMobReward;
import com.bth.models.reward.MobReward;
...
public class Mob {
    ...
    public long getTiemNangForPlayer(Player pl, long dame) {
        ...
        if (pl.zone.map.mapId == 149 || pl.zone.map.mapId == 150
                || pl.zone.map.mapId == 151 || pl.zone.map.mapId == 152
                || pl.zone.map.mapId == 153 || pl.zone.map.mapId == 154
                || pl.zone.map.mapId == 155 || pl.zone.map.mapId == 156
                || pl.zone.map.mapId == 157 || pl.zone.map.mapId == 158 || pl.zone.map.mapId == 159){
            tiemNang = 1;
        }
        if (pl.zone.map.type == 4){
            tiemNang *= 3;
        }
        return tiemNang;
    }
    ...
    private Player getPlayerCanAttack() {
        ...
        try {
            ...
            for (Player pl : players) {
                if (!pl.isNewPet &&!pl.isDie() && !pl.isBoss && !pl.effectSkin.isVoHinh) {
...
//        nplayer
        ...
        try {
            if ((!player.isPet &&!player.isNewPet && player.getSession().actived && player.setClothes.setDHD == 5) || (player.isPet && ((Pet) player).master.getSession().actived && ((Pet) player).setClothes.setDHD == 5)) {
                byte random = 1;
    ...
    public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
        ...
        if (player.itemTime.isUseMayDo && Util.isTrue(15, 150) && this.tempId > 57 && this.tempId < 66) {
        ...
        if (player.itemTime.isUseMayDo2 && Util.isTrue(15, 100) && this.tempId > 80 && this.tempId < 81) {
            list.add(new ItemMap(zone, 1110, 1, x, player.location.y, player.id));// cai nay sua sau nha
        }
        if(Util.isTrue(1,10000)){
            int itemDtl = Util.nextInt(555,567);
            int coler = Util.nextInt(105,110);
            if(this.zone.zoneId == coler) {
                Service.getInstance().dropItemMap(this.zone, new ItemMap(Util.ratiDTL(zone, itemDtl, 1, this.location.x, this.location.y, player.id)));
            }
            }
        if (player.setClothes.godClothes) {
            if (Util.isTrue(50, 2000)) {
                short[] thucan = new short[]{663,664,665,666,667};
                list.add(new ItemMap(zone, thucan[Util.nextInt(thucan.length - 1)], 1, x, player.location.y, player.id));//thuc an
            }
        }
        switch (Manager.SUKIEN)
        {
            case 0:
                if (Util.isTrue(10, 2000)) {
                    list.add(new ItemMap(zone, 1003, 1, x, player.location.y, player.id));//cá bảy màu
                }
                if (Util.isTrue(1, 2000)) {
                    list.add(new ItemMap(zone, 1004, 1, x, player.location.y, player.id));//cá diêu hồng
                }
                if (Util.isTrue(30, 2000)) {
                    list.add(new ItemMap(zone, 748, 1, x, player.location.y, player.id));//thịt heo
                }
                if (Util.isTrue(30, 2000)) {
                    list.add(new ItemMap(zone, 751, 1, x, player.location.y, player.id));//lá dong
                }
                if (Util.isTrue(15, 2000)) {
                    list.add(new ItemMap(zone, 749, 1, x, player.location.y, player.id));//thúng nếp
                }
                if (Util.isTrue(15, 2000)) {
                    list.add(new ItemMap(zone, 750, 1, x, player.location.y, player.id));//thúng đậu xanh
                }
                break;
            case 1:
                if (Util.isTrue(1, 2000)) {
                    list.add(new ItemMap(zone, 610, 1, x, player.location.y, player.id));//hoa hồng
                }
                break;
            case 2:
                if (Util.isTrue(1, 2000)) {
                    list.add(new ItemMap(zone, 1035, 1, x, player.location.y, player.id));//hoa hồng
                }
                break;
        }
        return list;
